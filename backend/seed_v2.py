...
Clears all data and re-seeds with a richer dataset.
"""
import sys, os
...
tech_services = [
    ...
    # Priya — HVAC specialist + Electrical
...
db.flush()
print(f"  Created {len(tech_services)} technician-service assignments")
...
jobs_data = [
    ...
    Job(customer_id=customers[0].id, technician_id=tech["Priya Patel"].id,
        ...
        scheduled_end_at=now - timedelta(days=3, hours=-3)),
...
availability_blocks = [
    ...
    AvailabilitySlot(
        ...
        slot_type="time_off",
        start_time=(now + timedelta(days=4)).replace(hour=0, minute=0),
        end_time=(now + timedelta(days=5)).replace(hour=0, minute=0),
        notes="Vacation day",
    ),
]
db.add_all(availability_blocks)
db.flush()
print(f"  Created {len(availability_blocks)} availability blocks")

# ── Invoices + Payments ───────────────────────────────────────────────────
paid_job = jobs_data[5]
inv = Invoice(
    job_id=paid_job.id,
    customer_id=paid_job.customer_id,
    subtotal=450.00,
    tax=36.00,
    total=486.00,
    status="paid",
...
db.add(inv)
db.flush()
db.add(InvoiceItem(invoice_id=inv.id, description="AC Unit Installation", quantity=1, unit_price=350.00, total=350.00))
db.add(InvoiceItem(invoice_id=inv.id, description="Refrigerant + Materials",quantity=1, unit_price=100.00, total=100.00))
...
db.commit()
print("  Created 2 invoices (1 paid, 1 draft)")

print("\nSeed complete!")
print("\nTechnician IDs for mobile app:")
for t in technicians:
    if t.status == "active":
        print(f"  {t.name} ({t.skill_level}): {t.id}")
db.close()
