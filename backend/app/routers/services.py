...
def assign_service_to_technician(
    service_id: uuid.UUID, tech_id: uuid.UUID, db: Session = Depends(get_db)
):
    svc = db.query(Service).filter(Service.id == service_id).first()
    if not svc:
        raise HTTPException(status_code=404, detail="Service not found")
    ...
    ts = TechnicianService(technician_id=tech_id, service_id=service_id)
    db.add(ts)
    db.commit()
...
def remove_service_from_technician(
    service_id: uuid.UUID, tech_id: uuid.UUID, db: Session = Depends(get_db)
):
